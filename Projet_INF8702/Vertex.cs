...
namespace Projet_INF8702
{
    ...
    public struct Vertex: IComparable<Vertex>
    {
        ...
        public Vertex(Vector3 position, Vector3 normal, Color color)
            : this(position, normal, color, Vector2.Zero, new Common.Mesh.SkinningVertex())
        { }
        ...
        public Vertex(Vector3 position, Vector3 normal, Color color, Vector2 uv)
            : this(position, normal, color, uv, new Common.Mesh.SkinningVertex())
        { }
        ...
        {
            var sqr1 = Position.LengthSquared();
            var sqr2 = v.Position.LengthSquared();
            //var angle = Math.Atan(Position.Z / Position.X) * 180 / Math.PI;
            //var angle2 = Math.Atan(v.Position.Z / v.Position.X) * 180 / Math.PI;



            //if (!angle.Equals(double.NaN)&&( (angle > 90 && angle < 270) || Math.Sign(angle) == -1)) sqr1 = -sqr1;
            //if (angle2!= double.NaN && ((angle2 > 90 && angle < 270) || Math.Sign(angle2) == -1)) sqr2 = -sqr2;
